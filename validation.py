    ...
    def run(self,dataset,max_epochs,num_configs=None) :
        ...
        with trange(num_configs,desc="Configs") as config_iter :
            for config_batch in config_iter :
                ...
                config_iter.set_postfix(validation_loss = avg_validation_loss, training_loss = avg_training_loss, val_deviation =  np.std(1.0/(len(batch_weights)*avg_validation_loss*batch_weights)))   
    ...
    def bootstrap_validation(self,params,dataset,max_epochs,model_file) :
        ...
        with trange(num_samples,desc="Bootstrap Samples") as samples_iter :
            for sample_id in samples_iter :
                ...
                samples_iter.set_postfix(avg_training_loss = avg_training_loss, avg_validation_loss = avg_validation_loss, val_deviation = np.std(1.0/(len(val_weights)*avg_validation_loss*val_weights)))
